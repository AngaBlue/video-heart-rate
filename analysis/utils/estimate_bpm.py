...
def estimate_bpm(signal: np.ndarray, fs: float) -> Optional[float]:
    ...
    if X.ndim == 1:
        ...
        raise ValueError(
            "signal must be 1D (T,) or 2D (T, C) with time along axis 0.")
    ...
    # FFT along time
    fft_vals = np.fft.fft(X, axis=0)              # (N, C)
    freqs = np.fft.fftfreq(N, d=1 / fs)          # (N,)
    ...
    peak_mag_per_col = band_mags[peak_idx_per_col,
                                 np.arange(band_mags.shape[1])]
    best_col = int(np.argmax(peak_mag_per_col))
    dom_freq = float(freqs_pos[band][peak_idx_per_col[best_col]])
    bpm = dom_freq * 60.0

    # --- Plot if HR is above threshold ---
    if ENABLE_PLOTTING and bpm > 80:
        plt.figure(figsize=(8, 4))
        for c in range(mags.shape[1]):
            plt.plot(freqs_pos * 60, mags[:, c], label=f"Channel {c}")
        plt.axvline(bpm, color="r", linestyle="--",
                    label=f"Peak ~ {bpm:.1f} BPM")
        plt.xlim(40, 180)  # physiological HR range
        plt.xlabel("Frequency (BPM)")
        plt.ylabel("Magnitude")
        plt.title("FFT Spectrum of ROI Signal")
        plt.legend()
        plt.tight_layout()
        plt.show()
